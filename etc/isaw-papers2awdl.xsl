...
<xsl:template match="h:a[@class='subject']">
 ...
 <xsl:template match="h:p[@id]">
  ...
  <xsl:element name="p">
   ...
   <xsl:apply-templates/><a id="{@id}anchor" class="id_link" style="color:aaa;display:none" href="#{@id}">↩</a><span class="id_label" id="{@id}anchor_label" style="color:aaa;display:none;position:fixed;right:0;bottom:50%">#<xsl:value-of select="@id"/>&#160;&#160;</span>
   </xsl:element>
 </xsl:template>
